...
Three pages describe distinct work at iFood, PicPay and Alloy / Kinter. Each has its own title, description, canonical URL, Article author and internal links. Engineering metrics come from the owner’s résumé and are scoped to the relevant work. Public company figures retain their separate dates and sources in the explorer. All five public pages appear in the sitemap and expose meaningful HTML without JavaScript. URLs use the exporter’s default no-trailing-slash form.
